  ...
  openModal({
    ...
    onOpen: (el, close) => {
      ...
      el.querySelector('[data-act="save"]').addEventListener('click', async () => {
        ...
        // Persistir stock por sucursal (crea el registro si es nuevo porque P.save inicializa qty=0)
        const prevLomas = p ? stockLomas : 0;
        const prevBanf  = p ? stockBanf  : 0;
        if (qtyLomas !== prevLomas) {
          const delta = qtyLomas - prevLomas;
          await P.adjustStock(saved.id, 'br_lomas', delta, p ? 'Ajuste manual desde edición' : 'Stock inicial');
        }
        if (qtyBanf !== prevBanf) {
          const delta = qtyBanf - prevBanf;
          await P.adjustStock(saved.id, 'br_banfield', delta, p ? 'Ajuste manual desde edición' : 'Stock inicial');
        }
