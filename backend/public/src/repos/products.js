...
export async function save(data) {
  ...
  const rec = {
    ...
    // Campos extendidos para Tienda Nube (todos opcionales)
    description: data.description || null,
    promotional_price: data.promotional_price != null && data.promotional_price !== '' ? Number(data.promotional_price) : null,
    weight: data.weight != null && data.weight !== '' ? Number(data.weight) : null,
    width: data.width != null && data.width !== '' ? Number(data.width) : null,
    height: data.height != null && data.height !== '' ? Number(data.height) : null,
    depth: data.depth != null && data.depth !== '' ? Number(data.depth) : null,
    seo_title: data.seo_title || null,
    seo_description: data.seo_description || null,
    handle: data.handle || null,
    video_url: data.video_url || null,
    tn_category_ids: Array.isArray(data.tn_category_ids) ? data.tn_category_ids.map(Number).filter((n) => !Number.isNaN(n)) : (before?.tn_category_ids || []),
